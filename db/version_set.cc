 ...
 private:
  ...
  InternalIterator* NewFileIterator(TableReader** table_reader_ptr = nullptr) {
    ...
    return table_cache_->NewIterator(
        ...
        block_protection_bytes_per_key_, &read_seq_, range_tombstone_iter_, promoted_ranges_);
  }
...
InternalIterator* Version::TEST_GetLevelIterator(
  ...
  auto level_iter = new (mem) LevelIterator(
      ...
      mutable_cf_options_.block_protection_bytes_per_key,
      nullptr /* range_del_agg */, nullptr, nullptr,
...
InternalIterator* Version::NewIterForLevel(
  ...
  return new (mem) LevelIterator(
      ...
      TableReaderCaller::kUserIterator, IsFilterSkipped(level), level,
      mutable_cf_options_.block_protection_bytes_per_key,
      /*range_del_agg=*/nullptr, nullptr, last_promoted,
...
void Version::AddIteratorsForLevel(const ReadOptions& read_options,
  ...
  if (level == 0) {
    ...
    for (size_t i = 0; i < storage_info_.LevelFilesBrief(0).num_files; i++) {
      ...
      auto table_iter = cfd_->table_cache()->NewIterator(
          ...
          *file.file_metadata, /*range_del_agg=*/nullptr,
          ...
          mutable_cf_options_.block_protection_bytes_per_key,
          /*range_del_read_seqno=*/nullptr, &tombstone_iter, nullptr,
          last_promoted);
      if (read_options.ignore_range_deletions) {
    ...
    // lazily.
    TruncatedRangeDelIterator*** tombstone_iter_ptr = nullptr;
    auto level_iter = NewIterForLevel(arena, read_options, soptions, level,
                                      allow_unprepared_value,
                                      &tombstone_iter_ptr, last_promoted);
    if (read_options.ignore_range_deletions) {
      merge_iter_builder->AddIterator(level_iter);
    } else {
      merge_iter_builder->AddPointAndTombstoneIterator(
...
void Version::HandleFound(const ReadOptions& read_options,
  ...
  if (get_context.is_blob_index() && get_context.do_merge() &&
      (value || columns)) {
    Slice blob_index =
        value ? *value
  ...
  if (GetContext::kMerge == get_context.State()) {
    ...
    if (value || columns) {
      ...
      status = MergeHelper::TimedFullMerge(
          merge_operator_, get_context.user_key(), MergeHelper::kNoBaseValue,
          get_context.merge_context().GetOperands(), info_log_, db_statistics_,
          clock_,
...
bool Version::GetInFile(EnvGet& env_get, GetContext& get_context,
  ...
  if (*get_context.max_covering_tombstone_seq() > 0) {
    ...
    // stop here.
    HandleNotFound(get_context, env_get.value, env_get.columns, env_get.status,
                   env_get.key_exists);
    return true;
  }
    ...
    case GetContext::kFound:
      if (env_get.db) {
        ...
        HandleFound(env_get.read_options, get_context, hit_level, env_get.value,
                    env_get.columns, env_get.status);
      }
      return true;
    ...
    if (env_get.db != nullptr) {
      ...
      if (level_pc->second.Get(cfd_->internal_stats(), env_get.k.user_key(),
        ...
        HandleFound(env_get.read_options, get_context, level_pc->first,
                    env_get.value, env_get.columns, env_get.status);
        return;
      }
    }
  }
...
void VersionStorageInfo::PrepareForVersionAppend(ColumnFamilyData* cfd,
  ...
  UpdateFilesByCompactionPri(cfd, version);
  GenerateFileIndexer();
...
void Version::PrepareAppend(const MutableCFOptions& mutable_cf_options,
  ...
  storage_info_.PrepareForVersionAppend(cfd_, *this);
}
        ...
        for (size_t i = 0; i < flevel->num_files; i++) {
          ...
          list[num++] = new InternalIterAppendLevel(
              0, cfd->table_cache()->NewIterator(
                ...
                /*range_del_iter=*/&range_tombstone_iter, &promoted_ranges));
        ...
        list[num++] = new InternalIterAppendLevel(
            ...
            new LevelIterator(
                ...
                c->input_version()->path_id(c->level(which)) == 0
                    ? &promoted_ranges
                    : nullptr,
                nullptr, c->boundaries(which), false, &tombstone_iter_ptr));
