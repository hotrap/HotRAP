...
Status CompactionOutputs::Finish(const Status& intput_status,
  ...
  if (s.ok()) {
    ...
    s = builder_->Finish(hot_per_byte_);
...
CompactionOutputs::CompactionOutputs(const Compaction* compaction,
                                     uint32_t path_id, double hot_per_byte,
                                     const bool is_penultimate_level)
    : compaction_(compaction), path_id_(path_id), hot_per_byte_(hot_per_byte),
      is_penultimate_level_(is_penultimate_level) {
