...
CompactionServiceCompactionJob::CompactionServiceCompactionJob(
    ...
    : CompactionJob(
          ...
          versions, shutting_down, log_buffer, nullptr,
          start_level_output_directory, latter_level_output_directory, nullptr,
          stats, db_mutex, db_error_handler, std::move(existing_snapshots),
          kMaxSequenceNumber, nullptr, nullptr, std::move(table_cache),
          event_logger, compaction->mutable_cf_options()->paranoid_file_checks,
...
Status CompactionServiceCompactionJob::Run() {
  ...
  compact_->sub_compact_states.emplace_back(
      ...
      compaction_input_.has_end ? std::optional<Slice>(end)
                                : std::optional<Slice>(),
      std::move(c->cached_ranges_to_promote()),
      /*sub_job_id*/ 0);
  ...
  if (status.ok()) {
    ...
    if (start_level_output_directory_) {
      io_s = start_level_output_directory_->FsyncWithDirOptions(
          IOOptions(), dbg, DirFsyncOptions());
    }
    if (io_s.ok() && latter_level_output_directory_) {
