...
CompactionIterator::CompactionIterator(
    ...
    bool must_count_input_entries, const SubcompactionState* sub_compact,
    ...
    : input_(input, cmp, must_count_input_entries),
      ...
      compaction_(sub_compact ? std::make_unique<RealCompaction>(
  ...
  if (sub_compact) {
    ...
    RALT* ralt = c.mutable_cf_options()->get_ralt();
    if (ralt && c.latter_level_path_id() > 0) {
      ...
      promotable_range_ = RangeBounds{
          .start = {.user_key = promotable_start, .excluded = false},
          .end = promotable_end,
      };

      hot_iter_.emplace(ralt->LowerBound(promotable_start));
    }
  }
