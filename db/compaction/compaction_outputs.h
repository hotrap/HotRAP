...
class CompactionOutputs {
 public:
  ...
  explicit CompactionOutputs(const Compaction* compaction, uint32_t path_id,
                             double hot_per_byte,
                             const bool is_penultimate_level);

  uint32_t path_id() const { return path_id_; }
  double hot_per_byte() const { return hot_per_byte_; }
  bool is_penultimate_level() const { return is_penultimate_level_; }
  ...
  void WritePromotedRanges(const std::vector<RangeSeq>& promoted_ranges) {
    builder_->WritePromotedRanges(promoted_ranges);
  }
