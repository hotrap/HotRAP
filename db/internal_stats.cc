...
void InternalStats::DumpCFStatsNoFileHistogram(bool is_periodic,
  ...
  for (const auto& cache : *caches) {
    ...
    value->append(
        "Promotion cache at L" + std::to_string(cache.first) + ": max size " +
        std::to_string(pc.max_size()) + ", max mut buffer size " +
        std::to_string(pc.mut_buffer_max_size()) + ", max range buffer size " +
