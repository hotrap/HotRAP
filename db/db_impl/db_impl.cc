  ...
  TieredIterator(Arena* arena, MergeIteratorBuilder& merge_iter_builder,
    ...
    if (first_level_in_slow_disk_ > 0) {
      ...
      version->AddIteratorsForLevel(
          ...
          last_level_in_fast_disk, allow_unprepared_value,
    ...
    for (int level = first_level_in_slow_disk_;
      ...
      InternalIterator* iter =
          version->NewIterForLevel(arena_, read_options_, file_options_, level,
                                   allow_unprepared_value_, &tombstone_iter_ptr);
      if (read_options_.ignore_range_deletions) {
        merge_iter_builder.AddIterator(iter);
      } else {
        merge_iter_builder.AddPointAndTombstoneIterator(
            iter, nullptr /* tombstone_iter */, tombstone_iter_ptr);
      }
    }
...
Status DBImpl::GetImpl(const ReadOptions& read_options, const Slice& key,
  ...
  if (!done) {
    ...
    sv->current->Get(
        this, read_options, lkey, get_impl_options.value, get_impl_options.columns,
...
Status DBImpl::GetLatestSequenceForKey(
  ...
  *found_record_for_key = false;
  ...
  if (!cache_only) {
    // Check tables
    PinnedIteratorsManager pinned_iters_mgr;
    sv->current->Get(this, read_options, lkey, /*value=*/nullptr, /*columns=*/nullptr,
                     timestamp, &s, &merge_context, &max_covering_tombstone_seq,
