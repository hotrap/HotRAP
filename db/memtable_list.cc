...
void MemTableList::PickMemtablesToFlush(uint64_t max_memtable_id,
  ...
  for (auto it = memlist.rbegin(); it != memlist.rend(); ++it) {
    ...
    // L0 files are sorted first by largest_seqno then by smallest_seqno, then
    // by number. Therefore, we shouldn't mix up imm memtables converted from a
    // mutable memtable and imm memtables generated by promotion by flush,
    // otherwise smallest_seqno will be small and the generated L0 SSTable can
    // be sorted behind an old SSTable.
    if (!ret->empty() && ((m->GetNextLogNumber() == 0) ^
                          (ret->back()->GetNextLogNumber() == 0))) {
      break;
    }
