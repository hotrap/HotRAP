...
Status BlockBasedTable::ReadPromotedRangeBlock(
  ...
  if (!s.ok()) {
    ...
    Status tmp_status;
    std::unique_ptr<InternalIterator> iter(NewDataBlockIterator<DataBlockIter>(
        read_options, handle, /*input_iter=*/nullptr,
        BlockType::kPromotedRanges, /*get_context=*/nullptr, lookup_context,
        prefetch_buffer, /*for_compaction=*/false, /*async_read=*/false,
        tmp_status, /*use_block_cache_for_lookup=*/true));
    assert(iter != nullptr);
    s = iter->status();
    if (!s.ok()) {
      ROCKS_LOG_WARN(
